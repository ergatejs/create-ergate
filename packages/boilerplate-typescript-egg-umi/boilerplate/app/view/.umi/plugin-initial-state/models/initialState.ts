...
const initState = {
  initialState: undefined as
    | ThenArg<ReturnType<typeof getInitialState>>
    | undefined,
  loading: true,
...
export default () => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const asyncFunc = () =>
        new Promise<ReturnType<typeof getInitialState>>((res) =>
          res(getInitialState()),
        );
  ...
  const setInitialState = useCallback(
    (initialState: ThenArg<ReturnType<typeof getInitialState>> | undefined) => {
